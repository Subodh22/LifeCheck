...
const COLORS = [
  "#4A9EE0", "#4CAF6B", "#C9A84C", "#E8A838",
  "#E85538", "#9B59B6", "#E8538A", "#6B6760",
...
export default function CreateAreaModal({ open, onClose, userId }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[580px] bg-[#111113] border border-[#2A2A2E] rounded-md shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#2A2A2E]">
          <div className="flex items-center gap-3">
            ...
            <span className="font-ui text-[13px] font-medium text-[#F2EEE8]">
              {step === "template" ? "Create area" : "Configure area"}
            </span>
          </div>
          <button type="button" onClick={onClose} className="text-[#6B6760] hover:text-[#F2EEE8] transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        {step === "template" && (
          <div className="p-5">
            <p className="font-ui text-[12px] text-[#6B6760] mb-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              {TEMPLATES.map((t) => (
                <button
                  ...
                  className="flex items-start gap-3 p-3 rounded border border-[#2A2A2E] bg-[#0D0D0F] hover:bg-[#18181B] hover:border-[#333338] transition-colors text-left"
                >
                  <span className="text-[18px] shrink-0 mt-0.5">{t.icon}</span>
                  <div className="min-w-0">
                    <p className="font-ui text-[13px] font-medium text-[#F2EEE8]">{t.name}</p>
            ...
            <button
              onClick={startBlank}
              className="w-full py-2 border border-dashed border-[#2A2A2E] rounded font-ui text-[12px] text-[#6B6760] hover:text-[#F2EEE8] hover:border-[#333338] transition-colors"
            >
              Start blank
            </button>
          </div>
        ...
        {step === "configure" && (
          <form onSubmit={handleSubmit}>
            <div className="p-5 space-y-4">
              {/* Preview */}
              <div className="flex items-center gap-3 p-3 bg-[#0D0D0F] border border-[#2A2A2E] rounded">
                <div
                ...
                <div>
                  <p className="font-ui text-[13px] font-medium text-[#F2EEE8]">{name || "Area name"}</p>
                  <p className="font-ui text-[11px] text-[#3A3A3E]">Key: {areaKey}</p>
                </div>
                {category && (
                  <span className="ml-auto font-ui text-[11px] text-[#6B6760] bg-[#18181B] border border-[#2A2A2E] px-2 py-0.5 rounded">
                    {category}
                  </span>
                )}
              </div>
              ...
              {/* Name */}
              <div>
                <label className="block font-ui text-[11px] tracking-[0.12em] uppercase text-[#6B6760] mb-1.5">
                ...
                <input
                  ...
                  required
                  className="w-full bg-[#18181B] border border-[#2A2A2E] rounded px-3 py-2 font-ui text-[13px] text-[#F2EEE8] placeholder:text-[#3A3A3E] outline-none focus:border-[#C9A84C] transition-colors"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#18181B] border border-[#2A2A2E] rounded px-3 py-2 font-ui text-[13px] text-[#F2EEE8] placeholder:text-[#3A3A3E] outline-none focus:border-[#333338] transition-colors"
                />
              </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {COLORS.map((c) => (
                    <button
                      ...
                      style={{
                        backgroundColor: c,
                        borderColor: color === c ? "#F2EEE8" : "transparent",
                      }}
                    >
